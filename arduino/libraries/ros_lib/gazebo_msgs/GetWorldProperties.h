  ...
  class GetWorldPropertiesResponse : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset++) = 0;
      ...
      uint32_t * length_model_namesi = (uint32_t *)(outbuffer + offset);
      *length_model_namesi = strlen( (const char*) this->model_names[i]);
      offset += 4;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      *val_sim_time |= ((uint32_t)(*(inbuffer + offset)) & 0x0f)<<19;
      ...
      for( uint8_t i = 0; i < model_names_length; i++){
      uint32_t length_st_model_names = *(uint32_t *)(inbuffer + offset);
      offset += 4;
