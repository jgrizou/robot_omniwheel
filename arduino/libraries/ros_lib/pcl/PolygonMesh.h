#ifndef _ROS_pcl_PolygonMesh_h
#define _ROS_pcl_PolygonMesh_h
...
namespace pcl
{
  ...
  class PolygonMesh : public ros::Msg
  {
    public:
      ...
      pcl::Vertices st_polygons;
      pcl::Vertices * polygons;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(polygons_lengthT > polygons_length)
        this->polygons = (pcl::Vertices*)realloc(this->polygons, polygons_lengthT * sizeof(pcl::Vertices));
      offset += 3;
      polygons_length = polygons_lengthT;
      for( uint8_t i = 0; i < polygons_length; i++){
      offset += this->st_polygons.deserialize(inbuffer + offset);
        memcpy( &(this->polygons[i]), &(this->st_polygons), sizeof(pcl::Vertices));
      }
     return offset;
    }

    const char * getType(){ return "pcl/PolygonMesh"; };
