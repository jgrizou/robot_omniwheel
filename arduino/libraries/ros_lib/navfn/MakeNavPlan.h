  ...
  class MakeNavPlanResponse : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 0) = (this->plan_found >> (8 * 0)) & 0xFF;
      ...
      uint32_t * length_error_message = (uint32_t *)(outbuffer + offset);
      *length_error_message = strlen( (const char*) this->error_message);
      offset += 4;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      offset += sizeof(this->plan_found);
      uint32_t length_error_message = *(uint32_t *)(inbuffer + offset);
      offset += 4;
