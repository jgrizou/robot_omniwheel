...
namespace visualization_msgs
{
  ...
  class InteractiveMarkerUpdate : public ros::Msg
  {
    ...
    {
      int offset = 0;
      uint32_t * length_server_id = (uint32_t *)(outbuffer + offset);
      *length_server_id = strlen( (const char*) this->server_id);
      offset += 4;
      ...
      *(outbuffer + offset++) = 0;
      ...
      uint32_t * length_erasesi = (uint32_t *)(outbuffer + offset);
      *length_erasesi = strlen( (const char*) this->erases[i]);
      offset += 4;
    ...
    {
      int offset = 0;
      uint32_t length_server_id = *(uint32_t *)(inbuffer + offset);
      offset += 4;
      ...
      for( uint8_t i = 0; i < erases_length; i++){
      uint32_t length_st_erases = *(uint32_t *)(inbuffer + offset);
      offset += 4;
