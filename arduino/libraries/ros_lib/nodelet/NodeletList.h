  ...
  class NodeletListResponse : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset++) = 0;
      ...
      uint32_t * length_nodeletsi = (uint32_t *)(outbuffer + offset);
      *length_nodeletsi = strlen( (const char*) this->nodelets[i]);
      offset += 4;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      for( uint8_t i = 0; i < nodelets_length; i++){
      uint32_t length_st_nodelets = *(uint32_t *)(inbuffer + offset);
      offset += 4;
